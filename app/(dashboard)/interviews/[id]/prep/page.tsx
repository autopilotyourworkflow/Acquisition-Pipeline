    ...
    <div className="space-y-6">
      <div>
        ...
        <div className="mt-2 flex flex-wrap items-end justify-between gap-3">
          <div>
            ...
            <h1 className="mt-1 font-display text-3xl font-medium text-black">
          ...
          <div className="text-right text-xs text-black">
            ...
            {interview.meet_url && (
              <a
                ...
                className="mt-1 inline-block text-[11px] font-medium text-black underline-offset-4 hover:underline"
              >
                Join Google Meet →
              </a>
            )}
      ...
      {prepQuestions.length > 0 ? (
        <section className="rounded-lg border border-soft-gray bg-white p-5">
          <h2 className="font-display text-xl text-black">Prep questions</h2>
          ...
          <ul className="mt-4 list-disc space-y-2 pl-6 text-sm text-black marker:text-black">
        ...
        <section className="rounded-lg border border-dashed border-soft-gray bg-white/40 p-6 text-center text-sm text-gray">
      ...
      {latestScore && (
        <section className="rounded-lg border border-soft-gray bg-white p-5">
          <div className="flex flex-wrap items-baseline justify-between gap-3">
            <h2 className="font-display text-xl text-black">Score snapshot</h2>
            ...
            <div className="mt-4 grid gap-4 md:grid-cols-2">
              {strengths.length > 0 && (
                <div>
                  ...
                  <ul className="mt-1.5 list-disc space-y-1 pl-5 text-sm text-black marker:text-success">
                    {strengths.map((s, i) => (
                      <li key={i}>{s}</li>
                    ))}
                  </ul>
              ...
              {gaps.length > 0 && (
                <div>
                  ...
                  <ul className="mt-1.5 list-disc space-y-1 pl-5 text-sm text-black marker:text-warning">
                    {gaps.map((g, i) => (
                      <li key={i}>{g}</li>
                    ))}
                  </ul>
      ...
      <section className="rounded-md border border-dashed border-soft-gray bg-white/40 p-4 text-xs text-gray">
...
function ScoreStat({
  ...
  return (
    <div className="rounded-md border border-soft-gray bg-white/40 px-3 py-2">
      <p className="text-[11px] uppercase tracking-wide text-black">
        {label}
      </p>
      <p className="mt-1 font-mono text-2xl font-medium text-black">
        {value.toFixed(1)}
      </p>
      {passes !== null && (
        <p
