...
| Redesign Phase 1+2 — token + font swap + hardcoded-color cleanup | 2026-05-21 | ✅ **COMPLETE.** Shipped invisibly during Phase 3 — one `globals.css` token rewrite + a `next/font/google` swap (Fraunces → Montserrat) flipped the whole app from navy/cream/terracotta to yellow/black/white in 30 minutes. Cowork-log entry #46 narrates the spine swap. All Phase 2 component cleanup also landed: `StageBadge` (8 stages on the new yellow ramp), `SourceBadge` (yellow underline for outbound), `ConflictWarning` (yellow-pale attention treatment), `login-form.client.tsx` CTAs (`bg-yellow text-black`). The lone surviving legacy alias `bg-charcoal/15` in `components/screener/ScoreCard.tsx` was migrated to `bg-soft-gray` alongside this status update. |
